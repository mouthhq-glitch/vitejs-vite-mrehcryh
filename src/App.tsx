...
function dbHeaders(){return{"apikey":SUPABASE_KEY,"Authorization":`Bearer ${SUPABASE_KEY}`,"Content-Type":"application/json","Prefer":"return=representation"};}
const db={
  async get(t,q=""){const r=await fetch(`${SUPABASE_URL}/rest/v1/${t}${q}`,{headers:dbHeaders()});if(!r.ok)throw new Error(await r.text());return r.json();},
  async upsert(t,b){const r=await fetch(`${SUPABASE_URL}/rest/v1/${t}`,{method:"POST",headers:{...dbHeaders(),"Prefer":"resolution=merge-duplicates,return=representation"},body:JSON.stringify(b)});if(!r.ok)throw new Error(await r.text());return r.json();},
  async insert(t,b){const r=await fetch(`${SUPABASE_URL}/rest/v1/${t}`,{method:"POST",headers:dbHeaders(),body:JSON.stringify(b)});if(!r.ok)throw new Error(await r.text());return r.json();},
  async del(t,q){const r=await fetch(`${SUPABASE_URL}/rest/v1/${t}${q}`,{method:"DELETE",headers:dbHeaders()});if(!r.ok)throw new Error(await r.text());},
};
...
const DEMO_EMP=[{id:1,name:"王小明",dept:"門市",position:"正職",hourly_rate:185,monthly_rate:28590,salary_type:"monthly"},{id:2,name:"李小花",dept:"外場",position:"兼職",hourly_rate:185,monthly_rate:0,salary_type:"hourly"},{id:3,name:"陳大偉",dept:"廚房",position:"正職",hourly_rate:200,monthly_rate:32000,salary_type:"monthly"},{id:4,name:"林美麗",dept:"行政",position:"工讀",hourly_rate:185,monthly_rate:0,salary_type:"hourly"}];
...
export default function App(){
  ...
  async function loadData(){if(demo){setEmployees(DEMO_EMP);return;}setLoading(true);try{const[emps,clocks,scheds]=await Promise.all([db.get("employees","?order=id"),db.get("clock_records",`?work_date=gte.${mp}-01&work_date=lte.${mp}-31`),db.get("schedules",`?work_date=gte.${mp}-01&work_date=lte.${mp}-31`)]);setEmployees(emps);const cm={};clocks.forEach(r=>cm[`${r.employee_id}_${r.work_date}`]=r);setClockMap(cm);const sm={};scheds.forEach(r=>sm[`${r.employee_id}_${r.work_date}`]=r.shift_id);setSchedMap(sm);}catch(e){toast_("連線失敗："+e.message,"error");}setLoading(false);}
  useEffect(()=>{if(user)loadData();},[user,vy,vm]);
  async function handleClock(empId,action){const t=new Date().toTimeString().slice(0,5);const key=`${empId}_${today}`;if(demo){setClockMap(prev=>{const r=prev[key]||{employee_id:empId,work_date:today};return{...prev,[key]:action==="in"?{...r,check_in:t}:{...r,check_out:t}};});toast_(action==="in"?"✅ 上班打卡成功":"👋 下班打卡成功");return;}try{const r=clockMap[key]||{};await db.upsert("clock_records",{employee_id:empId,work_date:today,check_in:action==="in"?t:r.check_in,check_out:action==="out"?t:r.check_out});await loadData();toast_(action==="in"?"✅ 上班打卡成功":"👋 下班打卡成功");}catch(e){toast_("打卡失敗","error");}}
  async function handleShift(empId,date,shiftId){setSchedMap(prev=>({...prev,[`${empId}_${date}`]:shiftId}));if(demo)return;try{await db.upsert("schedules",{employee_id:empId,work_date:date,shift_id:shiftId});}catch(e){toast_("排班儲存失敗","error");}}
  async function addEmp(){if(!newEmp.name.trim())return toast_("請輸入員工姓名","error");if(demo){setEmployees(p=>[...p,{...newEmp,id:Date.now(),hourly_rate:+newEmp.hourly_rate,monthly_rate:+newEmp.monthly_rate}]);setShowAdd(false);toast_("✅ 新增成功（Demo）");return;}try{await db.insert("employees",{name:newEmp.name,dept:newEmp.dept,position:newEmp.position,hourly_rate:+newEmp.hourly_rate,monthly_rate:+newEmp.monthly_rate,salary_type:newEmp.salary_type});await loadData();setShowAdd(false);toast_("✅ 員工新增成功");}catch(e){toast_("新增失敗","error");}}
  async function delEmp(id){if(demo){setEmployees(p=>p.filter(e=>e.id!==id));return;}try{await db.del("employees",`?id=eq.${id}`);await loadData();}catch(e){toast_("刪除失敗","error");}}
  function monthRecs(empId){return monthDays.map(d=>clockMap[`${empId}_${d}`]).filter(r=>r&&r.check_in);}
  const S={card:{background:"#1a2a3a",borderRadius:12,padding:"14px 16px",marginBottom:10,border:"1px solid #2a3a4a"},inp:{width:"100%",background:"#0f1923",border:"1px solid #2a3a4a",borderRadius:8,padding:"8px 10px",color:"#e8e0d0",fontSize:13,boxSizing:"border-box",fontFamily:"inherit"},sel:{width:"100%",background:"#0f1923",border:"1px solid #2a3a4a",borderRadius:8,padding:"8px 10px",color:"#e8e0d0",fontSize:13,fontFamily:"inherit"},nav:{background:"#2a3a4a",border:"none",color:"#e8e0d0",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:16}};
  const TABS=[{id:"clock",label:"⏰ 打卡",ownerOnly:false},{id:"schedule",label:"📅 排班",ownerOnly:false},{id:"salary",label:"💰 薪資",ownerOnly:true},{id:"employees",label:"👥 員工",ownerOnly:true}].filter(t=>!t.ownerOnly||isOwner);
  if(!user)return <Login onLogin={a=>{setUser(a);setTab("clock");}}/>;
  return(<div style={{minHeight:"100vh",background:"#0f1923",color:"#e8e0d0",fontFamily:"'Noto Sans TC',sans-serif"}}>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;700&display=swap" rel="stylesheet"/>
    <div style={{background:"linear-gradient(135deg,#1a2a3a,#0f1923)",borderBottom:"1px solid #2a3a4a",padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
      ...
      <div style={{flex:1}}><div style={{fontWeight:700,fontSize:16}}>台灣勞基法人資系統</div><div style={{fontSize:11,color:"#8a9ab0"}}>{today}{demo&&" （Demo）"}</div></div>
    ...
    {demo&&<div style={{background:"#2a1a0a",borderBottom:"1px solid #5a3a0a",padding:"8px 16px",fontSize:12,color:"#f0a500"}}>⚠ Demo 模式：資料不會儲存。</div>}
    {!isOwner&&<div style={{background:"#1a2a0a",borderBottom:"1px solid #3a4a2a",padding:"8px 16px",fontSize:12,color:"#8ab060"}}>🔒 店長模式：薪資資訊僅老闆可查看</div>}
    ...
    <div style={{padding:16,maxWidth:900,margin:"0 auto"}}>
      ...
      {tab==="clock"&&<div>
        ...
        {employees.map(emp=>{const rec=clockMap[`${emp.id}_${today}`]||{};return(<div key={emp.id} style={{...S.card,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:600,fontSize:15}}>{emp.name}</div><div style={{fontSize:12,color:"#8a9ab0"}}>{emp.dept}｜{emp.position}</div>
            <div style={{fontSize:12,marginTop:4,display:"flex",gap:14}}><span style={{color:rec.check_in?"#4caf50":"#555"}}>上班：{rec.check_in||"--:--"}</span><span style={{color:rec.check_out?"#f0a500":"#555"}}>下班：{rec.check_out||"--:--"}</span></div></div>
      ...
      {tab==="schedule"&&<div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>{SHIFTS.map(s=>(<span key={s.id} style={{background:s.color+"33",color:s.color,border:`1px solid ${s.color}66`,borderRadius:6,padding:"2px 10px",fontSize:12}}>{s.label}{s.start?` ${s.start}-${s.end}`:""}</span>))}</div>
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          <thead><tr><th style={{padding:"8px 6px",textAlign:"left",color:"#8a9ab0",borderBottom:"1px solid #2a3a4a",minWidth:70}}>員工</th>
            {monthDays.map(d=>{const day=parseInt(d.split("-")[2]);const iH=isHol(d)||isWk(d);return<th key={d} style={{padding:"4px 2px",textAlign:"center",color:iH?"#f0a500":"#8a9ab0",borderBottom:"1px solid #2a3a4a",minWidth:34}}>{day}{iH&&<div style={{fontSize:9}}>假</div>}</th>;})}
          </tr></thead>
          <tbody>{employees.map(emp=>(<tr key={emp.id}>
            ...
            {monthDays.map(d=>{const key=`${emp.id}_${d}`;const sid=schedMap[key]||"";const sh=SHIFTS.find(s=>s.id===sid);return(<td key={d} style={{padding:"2px",borderBottom:"1px solid #1a2a3a",textAlign:"center"}}>
              <select value={sid} onChange={e=>handleShift(emp.id,d,e.target.value)} style={{width:36,background:sh?sh.color+"44":"#2a3a4a",color:sh?sh.color:"#666",border:"1px solid #3a4a5a",borderRadius:4,fontSize:10,padding:"1px",fontFamily:"inherit",cursor:"pointer"}}>
                <option value="">-</option>{SHIFTS.map(s=><option key={s.id} value={s.id}>{s.id}</option>)}
              </select></td>);})}
          </tr>))}</tbody>
        </table></div>
      </div>}
      ...
      {tab==="salary"&&isOwner&&<div>
        ...
        {employees.map(emp=>{const recs=monthRecs(emp.id);const w=calcWage(emp,recs);return(<div key={emp.id} style={S.card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,fontSize:12}}>
            {[{l:emp.salary_type==="monthly"?"底薪":"正班薪資",v:`NT$ ${Math.round(w.base).toLocaleString()}`},{l:`正班 ${w.reg.toFixed(1)}h`,v:""},{l:`加班 ${(w.ot1+w.ot2).toFixed(1)}h`,v:`NT$ ${Math.round(w.ot).toLocaleString()}`},{l:`假日 ${w.hol.toFixed(1)}h`,v:`NT$ ${Math.round(w.hp).toLocaleString()}`},{l:"出勤天數",v:`${recs.length} 天`},{l:"時薪",v:`NT$ ${emp.hourly_rate}`}].map((x,i)=>(<div key={i} style={{background:"#0f1923",borderRadius:8,padding:"8px 10px"}}><div style={{color:"#8a9ab0",fontSize:11}}>{x.l}</div>{x.v&&<div style={{color:"#e8e0d0",fontWeight:600,marginTop:2}}>{x.v}</div>}</div>))}
          </div></div>);})}
      </div>}

      {tab==="employees"&&isOwner&&<div>
        ...
        {showAdd&&<div style={{...S.card,border:"1px solid #f0a500",marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[{l:"姓名",k:"name",t:"text"},{l:"時薪(元)",k:"hourly_rate",t:"number"},{l:"月薪(元)",k:"monthly_rate",t:"number"}].map(f=>(<div key={f.k}><div style={{fontSize:11,color:"#8a9ab0",marginBottom:4}}>{f.l}</div><input type={f.t} value={newEmp[f.k]} onChange={e=>setNewEmp(p=>({...p,[f.k]:e.target.value}))} style={S.inp}/></div>))}
            {[{l:"部門",k:"dept",o:DEPARTMENTS},{l:"職位",k:"position",o:POSITIONS}].map(f=>(<div key={f.k}><div style={{fontSize:11,color:"#8a9ab0",marginBottom:4}}>{f.l}</div><select value={newEmp[f.k]} onChange={e=>setNewEmp(p=>({...p,[f.k]:e.target.value}))} style={S.sel}>{f.o.map(o=><option key={o}>{o}</option>)}</select></div>))}
            <div><div style={{fontSize:11,color:"#8a9ab0",marginBottom:4}}>薪資類型</div><select value={newEmp.salary_type} onChange={e=>setNewEmp(p=>({...p,salary_type:e.target.value}))} style={S.sel}><option value="monthly">月薪制</option><option value="hourly">時薪制</option></select></div>
          </div>
        ...
        {employees.map(emp=>(<div key={emp.id} style={{...S.card,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:700,fontSize:15}}>{emp.name}</div><div style={{fontSize:12,color:"#8a9ab0"}}>{emp.dept}｜{emp.position}｜{emp.salary_type==="monthly"?"月薪":"時薪制"}</div>
            <div style={{fontSize:12,color:"#f0a500",marginTop:2}}>時薪：NT$ {emp.hourly_rate}{emp.salary_type==="monthly"?`　月薪：NT$ ${emp.monthly_rate?.toLocaleString()}`:""}</div></div>
          <button onClick={()=>delEmp(emp.id)} style={{background:"#2a1a1a",border:"1px solid #4a2a2a",color:"#e05b00",borderRadius:8,padding:"6px 12px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>刪除</button>
        </div>))}
      </div>}
