...
function ClockFixPopup({emp,date,current,onSave,onClose}){
  const[ci,setCi]=useState(current?.check_in||"");
  const[co,setCo]=useState(current?.check_out||"");
  return(
    <div style={{position:"fixed",inset:0,background:"#00000099",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#1a2a3a",borderRadius:16,padding:24,width:300,border:"1px solid #f0a500",boxShadow:"0 20px 60px #000"}} onClick={e=>e.stopPropagation()}>
        <div style={{fontWeight:700,fontSize:15,marginBottom:2}}>✏️ 修正打卡時間</div>
        <div style={{fontSize:12,color:"#8a9ab0",marginBottom:16}}>{emp.name}　{date}</div>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>onSave(ci,co)} style={{flex:1,background:"#f0a500",border:"none",color:"white",borderRadius:8,padding:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",fontSize:14}}>確認修正</button>
          <button onClick={onClose} style={{flex:1,background:"#2a3a4a",border:"none",color:"#e8e0d0",borderRadius:8,padding:11,cursor:"pointer",fontFamily:"inherit",fontSize:14}}>取消</button>
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  async function handleClockFix(emp,date,checkIn,checkOut){
    ...
    if(demo){setClockMap(prev=>({...prev,[key]:{...prev[key],check_in:checkIn,check_out:checkOut}}));setClockFixPopup(null);toast_("✅ 打卡時間已修正");return;}
    try{
      const r=clockMap[key]||{};
      if(r.id){
        await fetch(`${SUPABASE_URL}/rest/v1/clock_records?id=eq.${r.id}`,{method:"PATCH",headers:dbH(),body:JSON.stringify({check_in:checkIn,check_out:checkOut})});
      } else {
        await db.insert("clock_records",{employee_id:emp.id,work_date:date,check_in:checkIn,check_out:checkOut});
      }
      await loadData();setClockFixPopup(null);toast_("✅ 打卡時間已修正");
    }catch(e){toast_("修正失敗："+e.message,"error");}
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0f1923",color:"#e8e0d0",fontFamily:"'Noto Sans TC',sans-serif"}}>
      ...
      {clockFixPopup&&<ClockFixPopup
        ...
        date={clockFixPopup.date}
        current={clockMap[`${clockFixPopup.emp.id}_${clockFixPopup.date}`]}
        onSave={(ci,co)=>handleClockFix(clockFixPopup.emp,clockFixPopup.date,ci,co)}
        onClose={()=>setClockFixPopup(null)}
      />}
